...
const Dashboard = () => {
  ...
  const registerBtns = ["Google", "Facebook", "Email"];
        ...
        {!isAuthenticated ? (
          <>
            <section className="hero">
              ...
              <p>
                From local delicacies to international cuisines, get everything
        ...
        {showModal && (
          <Modal onClose={handleCloseModal}>
            ...
            <div className="flex gap-[10px]">
              {registerBtns.map((link,index)=>{
                return <Link to="/register" className="text-sky-600" key={index}>{link}</Link>
              })}
              <button onClick={handleCloseModal}>Skip for Now</button>
            </div>

            <p>
              By creating an account, you automatically accept our Terms of
