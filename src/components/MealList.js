...
const MealList = () => {
  ...
  const deleteMeal = async (mealId) => {
    try {
      const token = localStorage.getItem("access-token");
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.delete("http://localhost:5000/meals", {
        data: { id: mealId },
        ...config,
      });
      alert(response.data.message);
  ...
  const updateMeal = async () => {
    try {
      ...
      alert(response.data.message);
      setMeals(
        meals.map((meal) => (meal.id === formData.id ? formData : meal))
      );
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="block w-full px-4 py-2 mt-2 text-black bg-white border border-blue-400 rounded-md focus:ring-gray-700 focus:outline-none focus:ring focus:ring-opacity-40"
          />
        </div>
        <div>
      ...
      <ul>
        {meals &&
          meals.map((meal) => (
            <li key={meal.id}>
              <p className="text-sky-600">Name: {meal.name}</p>
